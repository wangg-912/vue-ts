...
class TagsView extends VuexModule implements ITagsViewsState {
     ...
     * 新增tagView的视图队列
     */
    @Mutation ADD_VISITED_VIEW(view : ITagsView) {
     ...
     * 新增tagView的视图名称
     */
    @Mutation ADD_CACHED_VIEW(view : ITagsView) {
     ...
     * 更新当前tagView的数据
     */
    @Mutation UPDATE_VISITED_VIEW(view : ITagsView) {
     ...
     * 删除当前tagView
     */
    @Mutation DEL_VISITED_VIEW(view : ITagsView) {
        for (const [i,
     ...
     * 删除当前缓存队列 tagName
     */
    @Mutation DEL_CACHED_VIEW(view : ITagsView) {
        for (const [i,
     ...
     */
    @Mutation DEL_OTHERS_VISITED_VIEW(view : ITagsView) {
     ...
     */
    @Mutation DEL_OTHERS_CACHED_VIEW(view : ITagsView) {
        for (const [i,
            v]of this.cachedviews.entries()) {
            if (v == view.path) {
